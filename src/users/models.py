...
class Action(models.Model):
    ...
    RECEIVE_ORDER = "получен заказ"  # исполнитель получил заказ
    CANCEL_ORDER = "отменен заказ"  # заказчик отменил заказ
    REJECT_ORDER = "отклонен заказ"  # исполнитель отказался от заказа
    ACCEPT_ORDER = "принят в работу заказ"  # исполнитель взял в работу заказ
    SUBMIT_ORDER = "сдан на проверку заказ"
    RETURN_ORDER = "возвращен на доработку заказ"
    COMPLETE_ORDER = (
        "успешно выполнил заказ"  # исполнитель успешно сдал работу заказчику
    )
    RECEIVE_RESULT = (
        "получил результат работы по заказу"  # заказчик получил результат работы
    )
